...
import { auth, db } from "../firebase"; // make sure firebase.js exports both auth & db
...
export default function Dashboard({ onLogout }) {
  ...
  // booking / feedback / ai states (kept from original)
  ...
  // Default user fallback values (used to merge with Firestore doc)
  const defaultUser = {
    fullName: "Patient",
    email: "",
    role: "patient",
    ...
    therapyProgress: 60,
    dominantDosha: "Vata",
    doshaBalance: { vata: 65, pitta: 25, kapha: 10 },
    currentTherapy: "Panchkarma",
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        try {
          ...
          if (snap.exists()) {
            // assume Firestore doc shape matches what the UI expects (or at least some fields)
            setUser(snap.data());
          } else {
          ...
          // still set a minimal user so the UI doesn't break
          setUser({
            fullName: firebaseUser.displayName || defaultUser.fullName,
            email: firebaseUser.email || "",
            role: "patient"
  ...
  const handleLogout = async () => {
    try {
      ...
      setSuccess("Failed to logout. Try again.");
  ...
  const getAIResponse = (message) => {
    ...
    if (lowerMessage.includes("appointment") || lowerMessage.includes("book")) {
      ...
      return "Your Ayurvedic constitution shows Vata dominance. This means you might benefit from grounding practices, warm oil treatments, and regular routines. Panchakarma therapy can help balance your doshas effectively.";
          ...
          <CardContent className="space-y-4">
            <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
              ...
              <div className="flex items-center gap-2 text-emerald-700">
                ...
                <span className="text-sm">Next: {profile.nextAppointment}</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            {Object.entries(profile.doshaBalance).map(([dosha, percentage]) => (
              <div key={dosha} className="space-y-2">
                ...
                <Progress value={Number(percentage)} className="h-2 bg-emerald-100 [&>div]:bg-emerald-500" />
              </div>
            ))}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
              ...
              <Select value={selectedTherapy} onValueChange={setSelectedTherapy}>
                ...
                <SelectContent className="bg-white z-50">
                  {therapyTypes.map((therapy) => (
                    <SelectItem key={therapy} value={therapy} className="focus:bg-gray-100 hover:bg-gray-100">
                      {therapy}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedDoctor} onValueChange={setSelectedDoctor}>
                ...
                <SelectContent className="bg-white z-50">
                  {doctors.map((doctor) => (
                    <SelectItem key={doctor.id} value={doctor.id} className="focus:bg-gray-100 hover:bg-gray-100">
                      <div>
                        <div>{doctor.name}</div>
                      </div>
                    </SelectItem>
            ...
            <div className="p-4 border border-emerald-200 rounded-lg">
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-emerald-600 text-sm">Dr. Amit Patel • Sep 15, 2025</p>
                </div>
                {renderStars(4)}
              </div>
  ...
  // If user is null - either loading or signed out. Show simple loading / empty state.
  if (user === null) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-lg text-emerald-700">Loading dashboard...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="absolute bottom-6 left-6 right-6">
            <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-emerald-800 text-sm truncate">{profile.fullName}</p>
                  <p className="text-emerald-600 text-xs">{profile.role}</p>
                </div>
              </div>
            </div>
            <Button 
